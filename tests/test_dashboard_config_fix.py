...
def test_dashboard_interval_consistency():
    ...
    try:
        ...
        if config_interval == fallback_interval:
            ...
            print(f"✗ Dashboard intervals are INCONSISTENT: config={config_interval}, fallback={fallback_interval}")
            return False
...
def test_worker_count_tracking():
    ...
    try:
        ...
        from adaptive_scaling_engine import set_current_worker_count, get_current_worker_count
        ...
        print(f"✓ Worker utilization: {unified_data.get('worker_utilization_percent', 0):.1f}%")
        ...
        print(f"✓ Active workers (target) after scaling: {unified_data_after.get('active_workers')}")
        print(f"✓ Busy workers (actual) after scaling: {unified_data_after.get('busy_workers')}")
        print(f"✓ Worker utilization after scaling: {unified_data_after.get('worker_utilization_percent', 0):.1f}%")
        ...
        if unified_data_after.get('active_workers') == 75:
            ...
            print(f"✗ Worker count changes NOT reflected: expected 75, got {unified_data_after.get('active_workers')}")
            return False
...
def test_dashboard_metrics_integration():
    ...
    try:
        ...
        print(f"✓ Dashboard worker utilization: {dashboard_data.get('worker_utilization', 0):.1f}%")
        ...
        if dashboard_data.get('active_workers') == 60:
            ...
            print(f"✗ Dashboard shows incorrect worker count: expected 60, got {dashboard_data.get('active_workers')}")
            return False
...
def main():
    ...
    if all(results):
        ...
        print("✗ Some tests failed - Dashboard configuration needs further investigation")
        return False
